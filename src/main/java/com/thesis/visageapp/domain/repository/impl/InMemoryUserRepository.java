...
public class InMemoryUserRepository implements UserRepository {
    ...
    public String updateUserData(User userDataToUpdate) throws SQLException {
        ...
        if (rs.next()) {
            ...
            if (user.getUserId().equals(StaticQueryParts.ERROR)) {
                ...
                String updateQuery = StaticQueryParts.updateUserData(userDataToUpdate);
    ...
    public String updateUserPassword(User userPart) throws SQLException {
        //MysqlConnector.connect();
        String resultCode = StaticQueryParts.RESPONSE_CODE_SUCCESS;
        User user = this.authenticateUser(userPart.getLogin(), userPart.getPassword());
        if (user.getUserId().equals(StaticQueryParts.ERROR)) {
            resultCode = StaticQueryParts.RESPONSE_CODE_ERROR_UPDATE_INCORRECT_PASSWORD;
        } else {
            String updateQuery = StaticQueryParts.updateUserPassword(userPart.getUserId(), userPart.getCity());
            MysqlConnector.executeOnDatabase(updateQuery);
        }
        MysqlConnector.disconnect();
        return resultCode;
    ...
    public String deleteUserAccount(User userToRemove) throws SQLException {
        String resultCode = StaticQueryParts.RESPONSE_CODE_SUCCESS;
        User user = this.authenticateUser(userToRemove.getLogin(), userToRemove.getPassword());
        if (user.getUserId().equals(StaticQueryParts.ERROR)) {
            resultCode = StaticQueryParts.RESPONSE_CODE_ERROR_UPDATE_INCORRECT_PASSWORD;
        } else {
            String removeQuery = StaticQueryParts.removeUser(userToRemove.getUserId());
